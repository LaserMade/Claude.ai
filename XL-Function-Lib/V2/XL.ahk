...
XL_Speedup(PXL,Status){ ;Helps COM functions work faster/prevent screen flickering, etc.
	if(!Status){
        ; ---------------------------------------------------------------------------
        ; @i...: the cursor must be set to prevent excel from updating it
        ; ---------------------------------------------------------------------------
		PXL.application.Cursor  := 2 
...
; ---------------------------------------------------------------------------
; @i...: Toggle screen visibility
; ---------------------------------------------------------------------------
XL_Screen_Visibility(PXL) => PXL.Visible:= ! PXL.Visible 
;***********************First row********************************.
;~ XL_First_Row(XL)
...
XL_Name_Delete_Name(PXL,Name) => PXL.Names(Name).Delete ;Delete a name
