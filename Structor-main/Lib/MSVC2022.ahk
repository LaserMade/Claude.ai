...
if (提示下载完整版)
{
  ...
  Content := "This tool needs to specify a C++ compiler to run properly.`r`nIt is recommended to download a full version with a compiler from below to use."
  Title := A_ScriptName
  MainIcon := 0xFFFF
  Flags := 0x10
  CustomButtons := []
  CustomButtons.Push([101, "Go to Github to download the full version."])
  CustomButtons.Push([102, "Go to AutoAHK to download the full version."])
  CustomButtons.Push([103, "I have a compiler."])
