...
Edit_CharFromPos(hEdit,X,Y,&r_CharPos:="",&r_LineIdx:="")  {
    Static Dummy3902
          ...
          ,EM_CHARFROMPOS        :=0xD7          ,EM_GETFIRSTVISIBLELINE:=0xCE          ,EM_LINEINDEX          :=0xBB          ;-- Messages
...
;
;       (start code)
; REMOVED: ;       SetBatchLines 100ms  ;-- Large bump in priority
;       RC:=Edit_ContainsSoftLineBreaks(hEdit)
; REMOVED: ;       SetBatchLines 10ms   ;-- Default priority
;       (end)
;
...
Edit_Convert2DOS(p_Text)    {
    ...
    p_Text := StrReplace(p_Text, "`r`n", "`n")             ;-- Convert DOS to Unix
    ; StrReplace() is not case sensitive
    ; check for StringCaseSense in v1 source script
    ; and change the CaseSense param in StrReplace() if necessary
    p_Text := StrReplace(p_Text, "`r", "`n")               ;-- Convert Mac to Unix
    ; StrReplace() is not case sensitive
    ; check for StringCaseSense in v1 source script
    ; and change the CaseSense param in StrReplace() if necessary
    p_Text := StrReplace(p_Text, "`n", "`r`n")             ;-- Convert Unix to DOS
    Return p_Text
    }
;-----------------------------
;
...
Edit_Convert2Mac(p_Text)    {
    ...
    ; check for StringCaseSense in v1 source script
    ; and change the CaseSense param in StrReplace() if necessary
    p_Text := StrReplace(p_Text, "`r`n", "`r")             ;-- Convert DOS to Mac
    ; StrReplace() is not case sensitive
    ; check for StringCaseSense in v1 source script
    ; and change the CaseSense param in StrReplace() if necessary
    p_Text := StrReplace(p_Text, "`n", "`r")               ;-- Convert Unix to Mac
    if StrLen(p_Text)
...
Edit_ConvertCase(hEdit,p_Case)     {
    p_Case := StrTitle(p_Case)  ;-- Just in case StringCaseSense is On
...
Edit_Disable(hEdit)
    {
    ControlSetEnabled(0,,"ahk_id " hEdit)
...
Edit_Enable(hEdit)
    {
    ControlSetEnabled(1,,"ahk_id " hEdit)
    Return ErrorLevel ? False:True
    }
...
Edit_EnableScrollBar(hEdit,wSBflags,wArrows)
    ...
    Static Dummy5401
          ...
          ,ESB_ENABLE_BOTH:=0x0          ;-- Scrollbar Arrows
...
Edit_GetActiveHandles(&hEdit:="",&hWindow:="",p_MsgBox:=False)
    ...
    if p_MsgBox
        MsgBox(262160)
            ,Error            ,This request cannot be performed on this control.  %A_Space%                ;-- 0x0 (OK button) + 0x10 ("Error" icon) + 0x40000 (AOT)
...
Edit_GetComboBoxEdit(hCombo)
    ...
    NumPut("UInt", cbSize, COMBOBOXINFO, 0)                ;-- cbSize
...
Edit_GetScrollBarInfo(hEdit,idObject)
    {
    Static Dummy4820          ,SCROLLBARINFO

          ,OBJID_HSCROLL:=0xFFFFFFFA          ,OBJID_VSCROLL:=0xFFFFFFFB          ,OBJID_CLIENT :=0xFFFFFFFC          ;-- Object identifiers

          ,STATE_SYSTEM_UNAVAILABLE:=0x1          ,STATE_SYSTEM_PRESSED    :=0x8          ,STATE_SYSTEM_INVISIBLE  :=0x8000          ,STATE_SYSTEM_OFFSCREEN  :=0x10000          ;-- rgstate flags
    ...
    NumPut("UInt", 60, SCROLLBARINFO, 0)                   ;-- cbSize
...
Edit_GetSel(hEdit,&r_StartSelPos:="",&r_EndSelPos:="")
    {
    Static Dummy3304          ,s_StartSelPos          ,s_EndSelPos          ,Dummy1:=s_StartSelPos := Buffer(4, 0)          ,Dummy2:=s_EndSelPos := Buffer(4, 0) ; V1toV2: if 's_EndSelPos' is a UTF-16 string, use 'VarSetStrCapacity(&s_EndSelPos, 4)'

          ,EM_GETSEL:=0xB0          ;-- Message
...
Edit_HasFocus(hEdit)
    {
    Static Dummy7291          ,GUITHREADINFO

          ,cbSize:=(A_PtrSize=8) ? 72:48          ,Dummy1:=VarSetStrCapacity(&GUITHREADINFO, cbSize)          ,Dummy2:=NumPut("UInt", cbSize, GUITHREADINFO, 0)          ;-- Create and initialize GUITHREADINFO structure ; V1toV2: if 'GUITHREADINFO' is NOT a UTF-16 string, use 'GUITHREADINFO := Buffer(cbSize)'
...
Edit_IsWordWrap(hEdit)
    ...
    Static Dummy8256
          ...
          ,ES_LEFT       :=0x0          ,ES_CENTER     :=0x1          ,ES_RIGHT      :=0x2          ,ES_MULTILINE  :=0x4          ,ES_AUTOHSCROLL:=0x80          ,WS_HSCROLL    :=0x100000          ;-- Styles
...
Edit_LineScroll(hEdit,xScroll:=0,yScroll:=0)
    ...
    Static Dummy3496
          ...
          ,SB_LEFT :=6          ,SB_RIGHT:=7          ;-- Horizontal scroll values

          ,SB_TOP   :=6          ,SB_BOTTOM:=7          ;-- Vertical scroll values

          ,EM_LINESCROLL:=0xB6          ,WM_HSCROLL   :=0x114          ,WM_VSCROLL   :=0x115          ;-- Messages
...
Edit_PosFromChar(hEdit,p_CharPos,&X:="",&Y:="")
    {
    Static Dummy5026          ,POINT

          ,EM_POSFROMCHAR:=0xD6          ;-- Message
...
Edit_ScrollPage(hEdit,p_HPages:=0,p_VPages:=0)
    ...
    Static Dummy3535
          ...
          ,SB_PAGELEFT :=2          ,SB_PAGERIGHT:=3          ;-- Horizontal scroll values

          ,SB_PAGEUP  :=2          ,SB_PAGEDOWN:=3          ;-- Vertical scroll values

          ,WM_HSCROLL :=0x114          ,WM_VSCROLL :=0x115          ;-- Messages
...
Edit_SetPasswordChar(hEdit,p_CharValue:=9679)
    ...
    WinRedraw("ahk_id " hEdit)  ;-- Force style change to show
    Return RC
    }
...
Edit_ShowBalloonTip(hEdit,p_Title,p_Text,p_Icon:=0)
    ...
    Static Dummy8144
          ...
          ,TTI_NONE         :=0          ,TTI_INFO         :=1          ,TTI_WARNING      :=2          ,TTI_ERROR        :=3          ;-- p_Icon values

          ,TTI_INFO_LARGE   :=4          ,TTI_WARNING_LARGE:=5          ,TTI_ERROR_LARGE  :=6          ;-- p_Icon values (Vista+ only)

          ,EM_SHOWBALLOONTIP:=0x1503          ;-- Messages
...
Edit_ShowScrollBar(hEdit,wBar,p_Show:=True)
    ...
    Static Dummy6622
          ...
          ,OBJID_HSCROLL:=0xFFFFFFFA          ,OBJID_VSCROLL:=0xFFFFFFFB          ;-- Object identifiers

          ,STATE_SYSTEM_UNAVAILABLE:=0x1          ,STATE_SYSTEM_PRESSED    :=0x8          ,STATE_SYSTEM_INVISIBLE  :=0x8000          ,STATE_SYSTEM_OFFSCREEN  :=0x10000          ;-- rgstate flags

          ,SB_HORZ:=0          ;-- Scroll bar flags
