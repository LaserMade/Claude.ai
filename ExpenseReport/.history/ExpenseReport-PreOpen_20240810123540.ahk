...
class ChromeRiverGUI {
    ...
    ; ---------------------------------------------------------------------------
    ; @example GUI Creation and Setup
    ; ---------------------------------------------------------------------------

    CreateGUI() {
        this.mainGui := Gui('+Resize +MinSize400x300')
        this.mainGui.Title := 'Chrome River Expense Report Helper'
    ...
    CreateMenuBar() {
        fileMenu := Menu()
        fileMenu.Add("&Save Settings", (*) => this.SaveSettings())
        fileMenu.Add("&Load Settings", (*) => this.LoadSettings())
    ...
    CreateMainContent() {
        ...
        this.mainGui.Add("Text", "xm y+10 w200", "Report Title:")
        this.reportTitle := this.mainGui.Add("Edit", "x+10 yp w300")

        ; Main action buttons
        buttonNames := ['Open Expense Report', 'Submit Expense Report', 'Select All Transactions', 'Add Selected to Report']
        buttonWidth := SetButtonLength(buttonNames) * 7.5
        
        this.buttons := {}
        for index, name in buttonNames {
            if (index == 1) {
                this.buttons[name] := this.mainGui.Add("Button", "xm y+20 w" buttonWidth " h30", name)
            } else {
                this.buttons[name] := this.mainGui.Add("Button", "x+10 yp w" buttonWidth " h30", name)
            }
            this.buttons[name].OnEvent("Click", (*) => this.HandleMainButton(name))
        }

        ; Unclassified expenses section
        this.mainGui.Add("Text", "xm y+20", "Unclassified Expenses:")
        this.expenseListView := this.mainGui.Add("ListView", "xm y+10 r5 w750", ["Description", "Amount", "Date", "Category"])
        
        navButtons := ['Previous', 'Select', 'Next']
        navButtonWidth := SetButtonLength(navButtons) * 7.5
        
        for index, name in navButtons {
            if (index == 1) {
                this.buttons[name] := this.mainGui.Add("Button", "xm y+10 w" navButtonWidth " h30", name)
            } else {
                this.buttons[name] := this.mainGui.Add("Button", "x+10 yp w" navButtonWidth " h30", name)
            }
            this.buttons[name].OnEvent("Click", (*) => this.HandleNavButton(name))
        }
    }
    ...
    CreateCategoriesSection() {
        ...
        xPos := this.mainGui.MarginX
        yPos := this.buttons["Next"].Pos.Y + this.buttons["Next"].Pos.H + 20
        
        for index, category in this.categoryOrder {
            if (Mod(index - 1, categoriesPerRow) == 0 && index > 1) {
                xPos := this.mainGui.MarginX
                yPos += 150  ; Adjust based on the height of category sections
            }
            
            this.mainGui.Add("Text", "x" xPos " y" yPos " w" maxButtonWidth, category)
            this.categoryButtons[category] := this.mainGui.Add("Button", "x" xPos " y+5 w" maxButtonWidth " h30", category)
            this.categoryButtons[category].OnEvent("Click", (*) => this.SelectCategory(category))
            
            classifications := this._categories.GetClassifications(category)
            classYPos := yPos + 40
            
            for cIndex, classification in classifications {
                if (cIndex > 3) {
                    break  ; Limit to 3 classification buttons per category for space
                }
                this.classificationButtons[classification] := this.mainGui.Add("Button", "x" xPos " y" classYPos " w" maxButtonWidth " h25", classification)
                this.classificationButtons[classification].OnEvent("Click", (*) => this.SelectClassification(category, classification))
                classYPos += 30
            }
            
            xPos += maxButtonWidth + buttonSpacing
        }
    }
    ...
    OnResize() {
        if (this.mainGui.Pos.W < 800) {
            ; Adjust for narrow width
            this.AdjustForNarrowWidth()
        } else {
            ; Adjust for wide width
            this.AdjustForWideWidth()
        }
        
        ; Recalculate positions of category buttons
        this.RepositionCategoryButtons()
    }

    AdjustForNarrowWidth() {
        ; Implementation for narrow width adjustments
    }

    AdjustForWideWidth() {
        ; Implementation for wide width adjustments
    }

    RepositionCategoryButtons() {
        ; Reposition category buttons based on new GUI width
    }

    HandleMainButton(name) {
        switch name {
    ...
    HandleNavButton(name) {
        switch name {
            case "Previous": this.NavigateExpense(-1)
            case "Select": this.SelectExpense()
            case "Next": this.NavigateExpense(1)
        }
    }

    ; ---------------------------------------------------------------------------
    ; @example Expense Management Methods
    ; ---------------------------------------------------------------------------
    ...
    UpdateExpenseDisplay() {
        this.expenseListView.Delete()
        for expense in this.unclassifiedExpenses {
            this.expenseListView.Add(, expense.description, expense.amount . ' ' . expense.currency, expense.date, expense.category)
        }
        this.expenseListView.ModifyCol()
        this.expenseListView.ModifyCol(1, 'AutoHdr')
    }

    NavigateExpense(direction) {
        ; Implementation for navigating expenses
    }

    SelectExpense() {
        ; Implementation for selecting an expense
    }

    SelectCategory(category) {
        ; Implementation for selecting a category
    }

    SelectClassification(category, classification) {
        ; Implementation for selecting a classification
    }

    ; ---------------------------------------------------------------------------
    ; @example Chrome River Interaction Methods
    ; ---------------------------------------------------------------------------
    ...
    SubmitExpenseReport() {
        ; Implementation for submitting the expense report
    }
    ...
    AddSelectedToReport() {
        ; Implementation for adding selected transactions to the report
    }

    FindControls() {
        ; Implementation for finding and parsing Chrome River controls
    }

    GetChromeRiverElement(&expRpt, title := 'Chrome River - Google Chrome') {
        ; Implementation for retrieving Chrome River element
    }
    ...
    ShowInfo(message) {
        ; Implementation for showing info messages
    }

    ToggleInfoDisplay() {
        ; Implementation for toggling info display
    }

    ToggleOutputMode() {
        ; Implementation for toggling output mode
    }
