...
class ChromeRiverGUI {
    ...
    CreateGUI() {
		...
		bNameLength := SetButtonLength(arrBtnNames)
        ...
        for category in this._categories.GetCategories() {
			iCategory := (Round((this._categories.GetCategories()).length))
			this.buttons.%category% := this.mainGui.AddButton('xm yp+40 w280 h30', category)
			; this.buttons.%category% := this.mainGui.AddButton('yp w280 h30', category)
			;  : 0
			; A_Index == 4 ? this.buttons.%category% := this.mainGui.AddButton('xm yp w280 h30', category) : 0
			; (A_Index <= 6 && A_Index > 4) ? this.buttons.%category% := this.mainGui.AddButton('yp w280 h30', category) : 0

			; while Round(A_Index < (this._categories.GetCategories()).length)
            ; this.buttons.%category% := this.mainGui.AddButton('xm yp+40 w280 h30', category)
			; this.buttons.%category%.OnEvent('Click', (*) => this.SelectCategory(category))
            yPos += 40
        }
        
        ; Sub-category buttons (classifications)
        xPos := 10
        yPos := yPos + 20
        maxButtonWidth := 140
        maxButtonsPerRow := 4
        ...
        for category in this._categories.GetCategories() {
            ...
            this.mainGui.AddText('x' . xPos . ' y' . yPos . ' w280', category)
            yPos += 30
            ...
            for classification in classifications {
                ...
                this.mainGui.AddButton('x' . xPos . ' y' . yPos . ' w' . maxButtonWidth . ' h30', classification).OnEvent('Click', (*) => this.SelectClassification(category, classification))
